...
// number of entries in pd and pt and other related constants
#define NUM_PD      1024
#define NUM_PT      1024
...
#define LOW10_ADDR_MASK     0x3FF
#define UPPER8_ADDR_MASK    0xFF
...
typedef struct page_directory_entry {
    ...
    uint8_t d       : 1;             // dirty
    uint8_t ps      : 1;             // page size = 1
    uint8_t g       : 1;             // global
    uint8_t avl     : 3;             // avl bits 11-9
    uint32_t addr   : 20;            // addr - for PS=1, need sl 1
...
typedef struct page_table_entry {
        ...
        uint8_t pcd : 1;             // cache disabled
        uint8_t a   : 1;             // accessed
        uint8_t d   : 1;             // dirty (page has been written to)
        uint8_t pat : 1;             // page attribute table
        uint8_t g   : 1;             // global
        uint8_t avl_9_11    : 3;
        uint32_t addr_31_12 : 20;    // page address
} __attribute__ ((packed)) pte_t;

// Page directory
pde_t pd[NUM_PD] __attribute__((aligned (PAGE_SIZE)));
// Page table
pte_t pt[NUM_PT] __attribute__((aligned (PAGE_SIZE)));

// user video memory page table
pte_t pt_user[NUM_PT] __attribute__((aligned (PAGE_SIZE)));
